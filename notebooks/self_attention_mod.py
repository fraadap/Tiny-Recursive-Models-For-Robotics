...
class TrainingConfig:
    ...
    trm_internal_layers: int = 2  # Numero di layer interni al TRMBlock (default 2 come nel paper)
    use_paper_architecture: bool = True  # Se True usa architettura TRM del paper con 3 token [x,y,z]

    def to_dict(self) -> Dict[str, Any]:
        return asdict(self)

    def label(self) -> str:
        arch = "TRM" if self.use_paper_architecture else "legacy"
        return f"{arch}_lr{self.lr}_h{self.hidden_dim}_rec{self.num_recursions}_bs{self.batch_size}"
...
class HyperparameterSearchSpace:
    ...
    # ===== TRM Paper Architecture Parameters =====
    trm_internal_layers: Optional[List[int]] = None
    use_paper_architecture: Optional[List[bool]] = None
    ...
    def as_optuna_space(self) -> Dict[str, List[Any]]:
        ...
        if self.trm_internal_layers is not None:
            space['trm_internal_layers'] = self.trm_internal_layers
        if self.use_paper_architecture is not None:
            space['use_paper_architecture'] = self.use_paper_architecture
        return space

# Numero 1
def default_search_space() -> HyperparameterSearchSpace:
    """Restituisce lo spazio di ricerca con supporto TRM paper architecture."""
    ...
    return HyperparameterSearchSpace(
        ...
        use_attention=[True],  # Abilitato per TRM paper
        attention_fusion=[False],
        # TRM Paper Architecture
        trm_internal_layers=[2, 4],  # 2 o 4 layer interni al TRMBlock
        use_paper_architecture=[True],  # Usa l'architettura TRM del paper
    )
...
# =============================================================================
# TRM ARCHITECTURE (Tiny Recursive Model - Paper-faithful implementation)
# =============================================================================
# L'architettura TRM usa 3 token:
#   - x (Input/Question): embedding dell'osservazione (visuale + opzionale testo)
#   - y (Prediction/Answer): token che evolve verso la predizione dell'azione
#   - z (Latent/Reasoning): token latente per ragionamento di medio-lungo termine
#
# Il blocco Transformer viene applicato ricorsivamente sulla sequenza [x, y, z]
# con self-attention tra i 3 token. Ad ogni passo:
#   1. Si costruisce la sequenza [x, y, z]
#   2. Si passa nel TRMBlock (Self-Attention + MLP)
#   3. Si estrae il nuovo y per predire l'azione (Reverse Embedding)
#   4. L'azione viene re-embeddata come nuovo y per il passo successivo
#   5. z viene aggiornato per accumulare ragionamento
# =============================================================================

class TRMBlock(nn.Module):
    """
    Blocco Transformer 'tiny' usato ricorsivamente nel TRM.
    
    Implementa l'architettura della figura del paper:
    - Self-Attention su sequenza di token [x, y, z]
    - Add & Norm (pre-norm style)
    - MLP (feed-forward)
    - Add & Norm
    
    Questo blocco viene riutilizzato per ogni passo ricorsivo.
    """
    ...
    def __init__(
        ...
        if self.use_attention:
            # Usa TransformerEncoder per avere più layer interni
            encoder_layer = nn.TransformerEncoderLayer(
                ...
                activation="gelu",
                batch_first=True,
                norm_first=True  # Pre-norm come nei transformer moderni
            )
            self.encoder = nn.TransformerEncoder(encoder_layer, num_layers=n_layers)
        else:
            # Fallback senza attention: solo MLP per ablation studies
            self.encoder = None
    ...
    def forward(self, tokens: torch.Tensor) -> torch.Tensor:
        """
        Args:
            tokens: (B, L, D) sequenza di token [x, y, z] o simile
        Returns:
            (B, L, D) token aggiornati dopo self-attention + MLP
        """
        if self.use_attention:
            return self.encoder(tokens)
        else:
            # Ablation: applica MLP indipendentemente a ogni token
            out = self.mlp(tokens)
            return self.norm(tokens + out)


# Manteniamo anche il vecchio RecursiveBlock per backward compatibility
class RecursiveBlock(nn.Module):
    """
    Blocco ricorsivo legacy (cross-attention style).
    Mantenuto per backward compatibility con checkpoint esistenti.
    """
...
class TRMPolicy(nn.Module):
    """
    Policy TRM (Tiny Recursive Model) per controllo robotico.
    
    Architettura fedele al paper TRM:
    =================================
    Usa 3 token che interagiscono via self-attention:
    
    - x (Input): embedding dell'osservazione (immagine + opzionale testo)
      Corrisponde alla "Question" nel paper originale.
    
    - y (Prediction): token che evolve verso la predizione dell'azione.
      Corrisponde alla "Answer" nel paper originale.
      Viene aggiornato ad ogni passo ricorsivo tramite "Reverse Embedding".
    
    - z (Latent): token latente per ragionamento di medio-lungo termine.
      Corrisponde al "Reasoning" nel paper originale.
      Accumula informazioni attraverso le iterazioni.
    
    Flow ricorsivo (applicato num_recursions volte):
    1. Costruisci sequenza [x, y, z] + type embeddings
    2. Passa nel TRMBlock (Self-Attention + Add&Norm + MLP + Add&Norm)
    3. Estrai y aggiornato → predici azione (Reverse Embedding)
    4. Re-embedding dell'azione come nuovo y (feedback loop)
    5. Aggiorna z per il prossimo passo
    
    NOTA: Input in formato PyTorch standard (B, C, H, W)
    """
    ...
    def __init__(
        ...
        use_attention: bool = True,
        attention_fusion: bool = False,
        trm_internal_layers: int = 2,  # Numero di layer interni al TRMBlock
        use_paper_architecture: bool = True  # Se True usa architettura TRM del paper
    ):
        super().__init__()
        ...
        if self.use_paper_architecture:
            # Architettura TRM del paper con 3 token [x, y, z]
            
            # Token iniziali learnable per y (prediction) e z (latent)
            self.y_token_init = nn.Parameter(torch.zeros(1, hidden_dim))
            self.z_token_init = nn.Parameter(torch.zeros(1, hidden_dim))
            nn.init.normal_(self.y_token_init, std=0.02)
            nn.init.normal_(self.z_token_init, std=0.02)
            
            # Type embeddings per distinguere i 3 token
            # 0: x (input), 1: y (prediction), 2: z (latent)
            self.token_type_embed = nn.Embedding(3, hidden_dim)
            
            # Blocco Transformer tiny riutilizzato ricorsivamente
            self.trm_block = TRMBlock(
                hidden_dim=hidden_dim,
                num_heads=num_heads,
                dropout=dropout,
                n_layers=trm_internal_layers,
                use_attention=use_attention
            )
            
            # Reverse Embedding: y_token → action
            ...
            # Re-embedding: action → y_token (feedback loop)
            self.action_to_token = nn.Sequential(
                nn.Linear(action_dim, hidden_dim),
                nn.GELU(),
                nn.LayerNorm(hidden_dim)
            )
            
            # Legacy block non usato in paper architecture
            self.recursive_block = None
        else:
            # Architettura legacy con cross-attention
            self.y_token_init = None
            self.z_token_init = None
            self.token_type_embed = None
            self.trm_block = None
            self.action_to_token = None
            
            self.recursive_block = RecursiveBlock(
                hidden_dim, num_heads, dropout, use_attention=self.use_attention
            )
    ...
    def _build_x_token(self, obs: torch.Tensor, prompts: Optional[List[str]] = None) -> torch.Tensor:
        """
        Costruisce il token x (Input) dall'osservazione.
        
        Args:
            obs: (B, C, H, W) immagini
            prompts: opzionale lista di prompt testuali
        Returns:
            x: (B, hidden_dim) token input
        """
        visual_features = self.encoder(obs)  # [B, visual_dim]
        ...
        if self.use_text_prompts:
            ...
            if self.attention_fusion:
                x = self.fusion_adapter(visual_features, text_features)
            else:
                x = self.fusion_adapter(torch.cat([visual_features, text_features], dim=-1))
        else:
            x = self.fusion_adapter(visual_features)
    ...
    def forward(
        ...
        """
        Forward pass del TRM.
        
        Args:
            obs: (B, C, H, W) osservazioni in formato CHW
            prompts: lista di stringhe (B) con la descrizione del task
            return_all_states: se True, restituisce tutti gli hidden states
            num_recursions: override del numero di ricorsioni (opzionale)
        Returns:
            actions: (B, action_dim) azioni predette
            (opzionale) all_states: dict con history di x, y, z tokens
        """
        B = obs.shape[0]
        R = num_recursions or self.num_recursions
        device = obs.device
        
        if self.use_paper_architecture:
            # ============ ARCHITETTURA TRM DEL PAPER ============
            # 1. Costruisci token x dall'osservazione
            x = self._build_x_token(obs, prompts)  # [B, D]
            
            # 2. Inizializza token y e z (replicate per batch)
            y = self.y_token_init.expand(B, -1).clone()  # [B, D]
            z = self.z_token_init.expand(B, -1).clone()  # [B, D]
            
            # Storage per history (se richiesto)
            ...
            # 3. Loop ricorsivo
            for t in range(R):
                # 3a. Costruisci sequenza [x, y, z]
                tokens = torch.stack([x, y, z], dim=1)  # [B, 3, D]
                
                # 3b. Aggiungi type embeddings
                type_ids = torch.arange(3, device=device).unsqueeze(0).expand(B, -1)  # [B, 3]
                tokens = tokens + self.token_type_embed(type_ids)
                
                # 3c. Passa nel TRMBlock (Self-Attention + MLP)
                tokens = self.trm_block(tokens)  # [B, 3, D]
                
                # 3d. Estrai token aggiornati
                x_new, y_new, z_new = tokens.unbind(dim=1)  # ciascuno [B, D]
                
                # 3e. Reverse Embedding: y → action
                action = self.action_head(y_new)  # [B, action_dim]
                
                # 3f. Re-embedding: action → nuovo y (feedback loop)
                y = self.action_to_token(action)  # [B, D]
                
                # 3g. Aggiorna z
                z = z_new
                
                # 3h. x può essere aggiornato o tenuto fisso
                # Nel paper originale x rimane tipicamente fisso
                # ma possiamo anche aggiornarlo per permettere
                # al modello di "reinterpretare" l'input
                x = x  # oppure x = x per tenerlo fisso
                
                # Storage
                if return_all_states:
                    all_states['x'].append(x.clone())
                    all_states['y'].append(y.clone())
                    all_states['z'].append(z.clone())
                    all_states['actions'].append(action.clone())
            
            # 4. Output finale: ultima azione predetta
            if return_all_states:
                return action, all_states
            return action
        
        else:
            # ============ ARCHITETTURA LEGACY (cross-attention) ============
            x_cond = self._build_x_token(obs, prompts)
            h = x_cond.clone()
...
def build_policy_from_config(
    ...
    use_paper_architecture: bool = True
) -> TRMPolicy:
    """
    Costruisce una TRMPolicy coerente con il TrainingConfig.
    
    Args:
        config: configurazione degli iperparametri
        obs_shape: shape dell'osservazione (C, H, W)
        use_paper_architecture: se True usa architettura TRM fedele al paper
                               con 3 token [x, y, z] e feedback ricorsivo;
                               se False usa architettura legacy cross-attention
    Returns:
        TRMPolicy configurata
    """
    return TRMPolicy(
        obs_shape=obs_shape,
        action_dim=7,
        ...
        trm_internal_layers=getattr(config, 'trm_internal_layers', 2),
        use_paper_architecture=getattr(config, 'use_paper_architecture', use_paper_architecture)
    )
...
def optuna_random_search(
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        trm_internal_layers_choices = search_space.trm_internal_layers or [2]
        use_paper_architecture_choices = search_space.use_paper_architecture or [True]
        ...
        trial_config = TrainingConfig(
            ...
            # TRM Paper Architecture parameters
            trm_internal_layers=trial.suggest_categorical('trm_internal_layers', trm_internal_layers_choices),
            use_paper_architecture=trial.suggest_categorical('use_paper_architecture', use_paper_architecture_choices),
            grad_clip=1.0,
...
def train_final_model(
    ...
    final_config = replace(
        base_config,
        epochs=30,
        save_path='models/paper_implementation.pt',
...
# %%
def main_pipeline(
    data_path: str = 'dataset/libero_goal',
    ...
    if train_final:
        ...
        if best_config is None or use_custom_final_config:
            best_config = TrainingConfig(
                ...
                double_visual_features=True,
                use_attention=True,
                attention_fusion=False,  # Disabilitato per TRM paper
                # ===== TRM Paper Architecture =====
                trm_internal_layers=2,  # 2 layer interni come nel paper
                use_paper_architecture=True  # Usa architettura TRM del paper con 3 token [x,y,z]
            )     
            print("⚠️  Usando configurazione TRM paper architecture di default")
...
# %%
main_pipeline(quick_search=False, train_final=True, evaluate=False, data_path='dataset/libero_spatial')
...
def evaluate_model(
    checkpoint_path: str = 'models/paper_implementation.pt',
...
for id in range(10):
    #id = 8
    final_results.append(evaluate_model(task_id=id, env_num=5, max_steps=600, save_videos=True)["success_rate"])
