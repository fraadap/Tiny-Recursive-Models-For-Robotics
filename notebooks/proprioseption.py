    ...
    try:
        with h5py.File(demo_file, 'r') as f:
            ...
            if 'obs' in demo_0:
                ...
                print(f"Available keys in obs_group: {list(obs_group.keys())}")  # Debug: print all keys
...
class LIBERODataset(Dataset):
    ...
    def __init__(
        ...
        # Dimensione dello stato proprio (da impostare durante il caricamento)
        self.proprio_dim = None  # Will be set when loading data
                ...
                with h5py.File(hdf5_file, 'r') as f:
                    ...
                    for demo_key in selected_demo_keys:
                        try:
                            ...
                            # Carica proprioception (joint positions)
                            proprio = None
                            proprio_keys = ['robot0_joint_pos', 'robot0_proprio_state', 'joint_pos']
                            proprio_key = next((k for k in proprio_keys if k in obs_group), None)
                            if proprio_key:
                                proprio = self._load_actions_robust(obs_group[proprio_key])
                                if self.proprio_dim is None:
                                    self.proprio_dim = proprio.shape[-1]
                                elif proprio.shape[-1] != self.proprio_dim:
                                    print(f"⚠️ Proprio dim mismatch in {hdf5_file.name}/{demo_key}, skipping...")
                                    continue
                            else:
                                # Use zeros as placeholder if proprio not found
                                proprio = np.zeros((len(actions), 7), dtype=np.float32)
                                if self.proprio_dim is None:
                                    self.proprio_dim = 7
                                print(f"⚠️ No proprio key found in {hdf5_file.name}/{demo_key}, using zeros...")
                            
                            # Verifica che obs, actions, proprio abbiano lunghezze compatibili
                            min_len = min(len(obs), len(actions), len(proprio))
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        proprio = demo['proprio'][start_idx:end_idx].copy()

        # Preprocessing
        obs = self._preprocess_obs(obs)
        actions = self._preprocess_actions(actions)
        proprio = self._preprocess_proprio(proprio)
    ...
    def _preprocess_obs(self, obs: np.ndarray) -> np.ndarray:
        return _libero_preprocess_obs_impl(self, obs)

    def _preprocess_actions(self, actions: np.ndarray) -> np.ndarray:
        """Normalizzazione azioni (z-score)"""
        if self.normalize_actions and self.action_stats['mean'] is not None:
            actions = (actions - self.action_stats['mean']) / self.action_stats['std']
        return actions.astype(np.float32)
    
    def _preprocess_proprio(self, proprio: np.ndarray) -> np.ndarray:
        """Preprocessing delle osservazioni proprioceptive"""
        # Normalizza o lascia come è (puoi aggiungere normalizzazione se necessario)
...
# %%
def _libero_preprocess_obs_impl(dataset: LIBERODataset, obs: np.ndarray) -> np.ndarray:
    processed = []
    ...
    obs = np.stack(processed, axis=0).astype(np.float32) / 255.0
    if dataset.augmentation:
        obs = _libero_augment_obs_impl(dataset, obs)
    return obs

def _libero_augment_obs_impl(dataset: LIBERODataset, obs: np.ndarray) -> np.ndarray:
    if np.random.rand() < 0.5:
        obs = np.clip(obs * np.random.uniform(0.8, 1.2), 0, 1)
    ...
    if np.random.rand() < 0.3:
        ...
        crop_h = int(dataset.image_size[0] * crop_ratio)
        crop_w = int(dataset.image_size[1] * crop_ratio)
        start_y = np.random.randint(0, dataset.image_size[0] - crop_h + 1)
        start_x = np.random.randint(0, dataset.image_size[1] - crop_w + 1)
        cropped = []
        for img in obs:
            img_crop = img[start_y:start_y+crop_h, start_x:start_x+crop_w]
            img_resized = cv2.resize(img_crop, (dataset.image_size[1], dataset.image_size[0]))
...
class TRMPolicy(nn.Module):
    ...
    def __init__(
        ...
        use_attention: bool = True,
        attention_fusion: bool = False,
        proprio_dim: int = 7  # Default proprio dimension (e.g., 7 for robot joints)
    ):
        super().__init__()
        ...
        if self.use_text_prompts:
            ...
            if self.attention_fusion:
                self.fusion_adapter = CrossAttentionFusion(
                    visual_dim=visual_dim + hidden_dim,  # Include proprio
                    text_dim=hidden_dim,
                    hidden_dim=hidden_dim,
                    num_heads=num_heads,
                    dropout=dropout
                ...
                fusion_in = visual_dim + hidden_dim + hidden_dim  # visual + proprio + text
            ...
            fusion_in = visual_dim + hidden_dim  # visual + proprio
    ...
    def forward(self, obs, proprio, prompts: Optional[List[str]] = None, return_all_states=False):
        """
        Args:
            obs: (B, C, H, W) osservazioni in formato CHW (PyTorch standard)
            proprio: (B, proprio_dim) proprioceptive input
        ...
        if self.use_text_prompts:
            ...
            if self.attention_fusion:
                x_cond = self.fusion_adapter(torch.cat([visual_features, proprio_features], dim=-1), text_features)
            else:
                x_cond = self.fusion_adapter(torch.cat([visual_features, proprio_features, text_features], dim=-1))
        else:
            x_cond = self.fusion_adapter(torch.cat([visual_features, proprio_features], dim=-1))
        h = x_cond.clone()
...
def build_policy_from_config(config: TrainingConfig, obs_shape: Tuple[int, int, int] = (3, 128, 128), proprio_dim: int = 7) -> TRMPolicy:
    ...
    def _train_epoch(self, epoch):
        ...
        for step, batch in enumerate(self.train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=self.use_amp):
                pred_actions = self.model(obs, proprio, prompts=prompts)
...
def train_final_model(
    ...
    model = build_policy_from_config(final_config, proprio_dim=proprio_dim)
    trainer = BehaviorCloningTrainer(
        model,
        train_loader,
        val_loader,
...
def evaluate_model(
    ...
    policy = build_policy_from_config(cfg, obs_shape=(3, camera_height, camera_width), proprio_dim=7).to(device)  # Assume proprio_dim=7, adjust if needed
        ...
        with VideoWriter(str(video_path), save_videos) as video_writer:
            for step in range(max_steps):
                ...
                with torch.no_grad():
                    ...
                    proprio_batch = torch.zeros((len(alive), 7), device=device)  # Placeholder, load actual proprio if available
                    actions_alive = policy(p_in, proprio_batch, prompt_batch)
