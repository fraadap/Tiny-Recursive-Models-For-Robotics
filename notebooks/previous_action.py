...
class TrainingConfig:
    ...
    action_fusion_method: Optional[str] = "concatenation"  # None, "concatenation", "cross_attention", "FiLM"

    def to_dict(self) -> Dict[str, Any]:
        return asdict(self)

    def label(self) -> str:
        action_method = self.action_fusion_method or "none"
        return f"lr{self.lr}_h{self.hidden_dim}_rec{self.num_recursions}_bs{self.batch_size}_action_{action_method}"
...
class HyperparameterSearchSpace:
    ...
    action_fusion_method: Optional[List[Optional[str]]] = None  # NEW: [None, "concatenation", "cross_attention", "FiLM"]
    ...
    def as_optuna_space(self) -> Dict[str, List[Any]]:
        ...
        if self.action_fusion_method is not None:
            space['action_fusion_method'] = self.action_fusion_method
        return space
...
def default_search_space() -> HyperparameterSearchSpace:
    ...
    return HyperparameterSearchSpace(
        ...
        action_fusion_method= [ "cross_attention"],  # NEW: Test all strategies
    )
...
class LIBERODataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        """
        Restituisce una transizione (osservazione, azione, azione precedente).
        
        Args:
            idx: indice della transizione
            
        Returns:
            Dict con 'observations', 'actions', e 'prev_actions' come tensori
        """
        ...
        # NEW: Get previous action (ground truth)
        if start_idx > 0:
            prev_actions = demo['actions'][start_idx - 1:start_idx].copy()
        else:
            # First frame: use zero action
            prev_actions = np.zeros((1, actions.shape[-1]), dtype=actions.dtype)

        # Preprocessing
        obs = self._preprocess_obs(obs)
        actions = self._preprocess_actions(actions)
        prev_actions = self._preprocess_actions(prev_actions)  # NEW: normalize with same stats

        # Per single-step prediction, restituisci solo primo elemento
        if self.sequence_length == 1:
            obs = obs[0]
            actions = actions[0]
            prev_actions = prev_actions[0]  # NEW
        ...
        return {
            ...
            'prev_actions': torch.from_numpy(prev_actions).float(),  # NEW
...
class CrossAttentionActionFusion(nn.Module):
    """Cross-attention between action history and visual+text features (Perceiver-Actor style)"""
    def __init__(self, visual_dim: int, text_dim: int, action_dim: int, hidden_dim: int, num_heads: int = 4, dropout: float = 0.1):
        super().__init__()
        # Note: action_dim parameter kept for compatibility, but we expect pre-encoded features (hidden_dim)
        self.action_proj = nn.Identity()  # No longer needed, actions pre-encoded
        self.visual_proj = nn.Linear(visual_dim, hidden_dim)
        self.text_proj = nn.Linear(text_dim, hidden_dim)
        
        # Cross-attention: action as query, visual+text as key/value
    ...
    def forward(self, visual_feats: torch.Tensor, text_feats: torch.Tensor, action_feats: torch.Tensor) -> torch.Tensor:
        """
        Args:
            visual_feats: (B, visual_dim)
            text_feats: (B, text_dim)
            action_feats: (B, hidden_dim) - PRE-ENCODED by action_encoder
        Returns:
            fused: (B, hidden_dim)
        """
        # Project visual and text to hidden_dim, action already encoded
        v = self.visual_proj(visual_feats).unsqueeze(1)  # (B, 1, D)
        t = self.text_proj(text_feats).unsqueeze(1)      # (B, 1, D)
        a = action_feats.unsqueeze(1)  # (B, 1, D) - already in hidden_dim
        
        # Context: visual + text
        context = torch.cat([v, t], dim=1)  # (B, 2, D)
        
        # Action attends to context
        attended, _ = self.cross_attn(a, context, context)
        
        # Residual + FFN
        fused = self.norm1(a + attended)
        fused = self.norm2(fused + self.ffn(fused))
        
        return fused.squeeze(1)  # (B, D)

class FiLMActionConditioner(nn.Module):
    """FiLM (Feature-wise Linear Modulation) conditioning with action history (BC-Z style)"""
    def __init__(self, visual_dim: int, text_dim: int, action_dim: int, hidden_dim: int, dropout: float = 0.1):
        super().__init__()
        # Combine visual and text first
        self.visual_text_fusion = nn.Sequential(
            nn.LayerNorm(visual_dim + text_dim),
            nn.Linear(visual_dim + text_dim, hidden_dim),
            nn.GELU(),
            nn.Dropout(dropout)
        )
        
        # Action to FiLM parameters (gamma and beta)
        # Note: action_dim parameter kept for compatibility, but we expect pre-encoded features (hidden_dim)
        self.action_to_film = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),  # Now expects encoded actions
            nn.GELU(),
            nn.Dropout(dropout),
    ...
    def forward(self, visual_feats: torch.Tensor, text_feats: torch.Tensor, action_feats: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            ...
            action_feats: (B, hidden_dim) - PRE-ENCODED by action_encoder
        Returns:
            modulated: (B, hidden_dim)
        """
        # Fuse visual and text
        visual_text = self.visual_text_fusion(torch.cat([visual_feats, text_feats], dim=-1))
        
        # Generate FiLM parameters from action
        film_params = self.action_to_film(action_feats)
        gamma, beta = torch.chunk(film_params, 2, dim=-1)
        
        # Apply FiLM: out = gamma * visual_text + beta
        modulated = gamma * visual_text + beta
        
        return self.out_norm(modulated)
...
class TRMPolicy(nn.Module):
    ...
    def __init__(
        ...
        self.action_fusion_method = action_fusion_method  # NEW
        
        # Action encoder (used by all action fusion strategies)
        if action_fusion_method in ["concatenation", "cross_attention", "FiLM"]:
        ...
        # Select fusion strategy based on action_fusion_method
        if action_fusion_method == "cross_attention":
            # Cross-attention between action and visual+text
            if not use_text_prompts:
                raise ValueError("cross_attention action fusion requires use_text_prompts=True")
            self.fusion_adapter = CrossAttentionActionFusion(
        ...
        elif action_fusion_method == "FiLM":
            # FiLM conditioning
            if not use_text_prompts:
                raise ValueError("FiLM action fusion requires use_text_prompts=True")
            self.fusion_adapter = FiLMActionConditioner(
                visual_dim=visual_dim,
                text_dim=hidden_dim,
                action_dim=action_dim,
                hidden_dim=hidden_dim,
        ...
        elif action_fusion_method == "concatenation":
            # Simple concatenation
            if use_text_prompts:
                if self.attention_fusion:
                    # Use text-visual cross attention, then concatenate action
                    self.text_visual_fusion = CrossAttentionFusion(
                        visual_dim=visual_dim,
                        text_dim=hidden_dim,
                        hidden_dim=hidden_dim,
                        num_heads=num_heads,
                    ...
                    fusion_in = hidden_dim + hidden_dim  # fused + action
                else:
                    fusion_in = visual_dim + hidden_dim + hidden_dim  # visual + text + action
            else:
                fusion_in = visual_dim + hidden_dim  # visual + action
        ...
        else:  # action_fusion_method is None
            # No action history, original architecture
            if use_text_prompts:
                if self.attention_fusion:
    ...
    def forward(self, obs, prompts: Optional[List[str]] = None, prev_actions: Optional[torch.Tensor] = None, return_all_states=False):
        """
        Args:
            obs: (B, C, H, W) osservazioni in formato CHW (PyTorch standard)
            prompts: lista di stringhe (B) con la descrizione del task
            prev_actions: (B, action_dim) azioni precedenti (ground truth in train, predette in eval)
        ...
        # Initialize prev_actions to zero if None
        if prev_actions is None and self.action_fusion_method is not None:
            prev_actions = torch.zeros(B, self.action_dim, device=obs.device)
        
        # Apply fusion based on action_fusion_method
        if self.action_fusion_method == "cross_attention":
            # Cross-attention fusion (action attends to visual+text)
            action_features = self.action_encoder(prev_actions)
            x_cond = self.fusion_adapter(visual_features, text_features, action_features)
            
        elif self.action_fusion_method == "FiLM":
            # FiLM conditioning (action modulates visual+text)
            action_features = self.action_encoder(prev_actions)
            x_cond = self.fusion_adapter(visual_features, text_features, action_features)
            
        elif self.action_fusion_method == "concatenation":
            # Simple concatenation
            action_features = self.action_encoder(prev_actions)
            
            if self.use_text_prompts:
                if self.attention_fusion:
                    # First fuse visual and text with attention
                    visual_text_fused = self.text_visual_fusion(visual_features, text_features)
                    # Then concatenate with action
                    x_cond = self.fusion_adapter(torch.cat([visual_text_fused, action_features], dim=-1))
                else:
                    # Simple concatenation of all three
                    x_cond = self.fusion_adapter(torch.cat([visual_features, text_features, action_features], dim=-1))
            else:
                # No text, just visual + action
                x_cond = self.fusion_adapter(torch.cat([visual_features, action_features], dim=-1))
                
        else:  # action_fusion_method is None
            # Original architecture without action history
            if self.use_text_prompts:
                if self.attention_fusion:
...
def build_policy_from_config(config: TrainingConfig, obs_shape: Tuple[int, int, int] = (3, 128, 128)) -> TRMPolicy:
    ...
    return TRMPolicy(
        ...
        action_fusion_method=getattr(config, 'action_fusion_method', None)  # NEW
    )
...
class BehaviorCloningTrainer:
    ...
    def train(self):
        ...
        for epoch in range(self.config.epochs):
            ...
            if val_metrics['loss'] < self.best_val_loss:
                ...
                # Generate filename based on action fusion method
                action_method = getattr(self.config, 'action_fusion_method', None) or 'none'
                base_path = self.best_model_path.replace('.pt', f'_{action_method}.pt')
                ...
                print(f"  ✓ Saved best model to {base_path} (val_loss: {val_metrics['loss']:.4f})")
                self._epochs_no_improve = 0
            else:
    ...
    def _train_epoch(self, epoch):
        ...
        for step, batch in enumerate(self.train_loader):
            ...
            prev_actions = batch['prev_actions'].to(self.device, non_blocking=True)  # NEW: Ground truth
            prompts = batch.get('prompt')

            self.optimizer.zero_grad(set_to_none=True)

            with torch.cuda.amp.autocast(enabled=self.use_amp):
                pred_actions = self.model(obs, prompts=prompts, prev_actions=prev_actions)  # NEW
    ...
    def _validate_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for batch in self.val_loader:
                ...
                prev_actions = batch['prev_actions'].to(self.device)  # NEW: Ground truth
                prompts = batch.get('prompt')
...
def optuna_random_search(
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        trial_config = TrainingConfig(
            ...
            action_fusion_method=trial.suggest_categorical('action_fusion_method', search_space.action_fusion_method or [None]),  # NEW
            grad_clip=1.0,
...
def train_final_model(
    ...
    final_config = replace(
        base_config,
        epochs=final_epochs,
        save_path='models/previous_action.pt',
    ...
    # Load best checkpoint with correct name
    action_method = getattr(final_config, 'action_fusion_method', None) or 'none'
    checkpoint_path = final_config.save_path.replace('.pt', f'_{action_method}.pt')
    ...
    if train_final:
        ...
        if best_config is None or use_custom_final_config:
            best_config = TrainingConfig(
                lr=0.0001,  # Non specificato nel JSON, assumo un valore di default o da aggiungere se necessario
                hidden_dim=256,
                num_recursions=16,
                epochs=12,
                batch_size=512,
                ...
                attention_fusion=False,
                action_fusion_method="concatenation"  # NEW: Use concatenation by default
            )     
...
# %%
main_pipeline(quick_search=False, train_final=False, evaluate=False)
...
def evaluate_model(
    checkpoint_path: str = 'optuna_trial_0_cross_attention.pt',
    ...
    video_skip: int = 1,
    debug_actions: bool = False  # NEW: Enable debug prints for action ranges
) -> Dict[str, Any]:
    ...
    try:
        ...
        # NEW: Buffer for previous actions (autoregressive inference)
        prev_actions_buffer = torch.zeros(env_num, action_dim, device=device)
        ...
        with VideoWriter(str(video_path), save_videos) as video_writer:
            for step in range(max_steps):
                ...
                # NEW: Get previous actions for alive environments
                prev_actions_alive = prev_actions_buffer[alive]

                with torch.no_grad():
                    prompt_batch = [task_prompt for _ in alive] if use_prompts else None
                    actions_alive = policy(
                        p_in, 
                        prompts=prompt_batch,
                        prev_actions=prev_actions_alive  # Autoregressive (input: normalized)
                    )
                    # Model output is already normalized
                    
                    # Debug: Check action ranges to verify fix
                    if debug_actions and step % 50 == 0 and len(alive) > 0:
                        print(f"\n[Debug] Step {step}:")
                        print(f"  prev_actions (input, normalized): min={prev_actions_alive.min().item():.3f}, max={prev_actions_alive.max().item():.3f}, mean={prev_actions_alive.mean().item():.3f}")
                        print(f"  pred_actions (output, normalized): min={actions_alive.min().item():.3f}, max={actions_alive.max().item():.3f}, mean={actions_alive.mean().item():.3f}")
                    
                    # ✅ FIX: Update buffer BEFORE denormalization
                    # Store normalized actions directly from model output
                    prev_actions_buffer[alive] = actions_alive.detach()
                    
                    # Denormalize ONLY for environment execution
                    actions_alive = actions_alive * action_std + action_mean
                    
                    if debug_actions and step % 50 == 0 and len(alive) > 0:
                        print(f"  denorm_actions (for env): min={actions_alive.min().item():.3f}, max={actions_alive.max().item():.3f}, mean={actions_alive.mean().item():.3f}")
                
                full_actions = np.zeros((env_num, action_dim), dtype=np.float32)
                full_actions[alive] = actions_alive.cpu().numpy().astype(np.float32)
